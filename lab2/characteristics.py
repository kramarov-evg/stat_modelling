...
def _get_steps(randoms: np.array, polygon: np.array) -> np.array:
    steps = np.zeros(13)
    for i in range(len(polygon)):
        steps[1+i] = sum(polygon[:i+1])
    return steps, [min(randoms) + (max(randoms) - min(randoms))/12*i for i in range(13)]
...
def get_specs(randoms: np.ndarray, full=False) -> dict:
    ...
    if full:
        ...
        steps, borders = _get_steps(polygon, randoms)
        specs['steps'] = steps
